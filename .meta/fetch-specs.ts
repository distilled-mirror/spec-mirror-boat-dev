 ...
 *
 * Boat publishes ONE OpenAPI 3.1 document — the same YAML its docs site
 * renders at https://docs.boat.dev/api/v1 (also listed from
 * https://docs.boat.dev/llms.txt as `/openapi/boat-v1.yaml`). There is no
...
const OPENAPI_SPEC_URL = "https://docs.boat.dev/openapi/boat-v1.yaml";
...
async function main() {
  ...
  const response = await fetch(OPENAPI_SPEC_URL, {
    headers: {
      ...
      "user-agent": "distilled.cloud-boat-dev-spec-mirror",
    },
  });
